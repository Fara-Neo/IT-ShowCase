...
- [x] **Список проектов** — таблица, edit/delete, подтверждение удаления
- [x] **Форма создания/редактирования проекта** — `ProjectForm` + `ProjectImageUpload` + категории/стек/публикация
...
2. **Подключить Mailtrap/Nodemailer** — заполнить SMTP-настройки и протестировать отправку заявки end-to-end.
3. **Доделать admin-панель заявок** — список, фильтрация, смена статуса.
4. **Доделать admin-панель пользователей** — список + смена ролей.
5. **Добавить CI/CD** — базовые проверки lint/build/migrate в GitHub Actions.
6. **Реализовать rate limiting для формы заявки** — закрыть High-priority issue по спаму.
